...
from .models import Project, UserProfile, Log, Consumable, Reagent, \
        Equipment, Sample, TrashConsumable, TrashReagent, TrashEquipment, \
        TrashSample
from .forms import UserApplicationForm, ProjectManagerSignUpForm, \
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            message1 = (
                    ...
                    "as it is reviewed. We will get back to you!"
                    )
            message2 = (
                    "If you do not receive a mail in the next 24 hours, "
                    "do well to reach out via mail."
                    )
            return render(
                    request,
...
def register_project_manager(request):
    ...
    return render(
            request,
            'inventory/register_project_manager.html',
            {
                'form': form,
                'email': email,
                'first_name': first_name,
    ...
    try:
        if request.method == 'POST':
            ...
            try:
                ...
                message1 = "Sorry, we couldn't find that email in out records."
                message2 = (
                        "Kindly confirm that the individual you are "
                        "trying to add is registered."
                )
                return render(
                        request,
                        'inventory/404_page.html',
            ...
            if (
                    project.project_editors.filter(email=email).exists()
                    or project.project_members.filter(email=email).exists()
            ):
                message1 = "This individual is already part of the project."
                message2 = (
                ...
                Log.objects.create(
                        project=project,
                        user=request.user,
                        action=f'{user.first_name} {user.last_name} added')
...
def remove_user(request, project_name, id):
    try:
        ...
        Log.objects.create(
                project=project,
                user=request.user,
                action=f'{user.first_name} {user.last_name} removed'
        )
...
def edit_user_access(request, project_name, id):
    ...
    return JsonResponse({
        'error': f'Error updating user with ID {id}.'}, status=400
    )
...
def create_project(request):
    if request.method == 'PUT':
        ...
        Log.objects.create(
                project=project,
                user=request.user,
                action=f'{project_name} added to Projects'
        )
    ...
    # Return a JSON response indicating failure
    return JsonResponse({"error": "Error creating project"}, status=400)
    ...
    try:
        if request.method == 'POST':
            ...
            Log.objects.create(
                    project=project,
                    user=request.user,
                    action=f'{name} added to Consumables'
            )
...
def dashboard_consumables(request, project_name):
    ...
    return render(
            request,
            'inventory/dashboard_consumables.html',
            {
                'plot_data': plot_data,
                'project': project,
                'projects': projects,
...
def edit_consumable(request, id):
    ...
    if request.method == 'PUT':
        ...
        consumable.product_code = data.get(
                'product_code', consumable.product_code
        )
        consumable.pack_size = data.get('pack_size', consumable.pack_size)
        consumable.quantity = data.get('quantity', consumable.quantity)
        consumable.expiry_date = data.get(
                'expiry_date ', consumable.expiry_date
        )
        consumable.storage_location = data.get(
                'storage_location', consumable.storage_location
        )
        consumable.threshold_value = data.get(
                'threshold_value', consumable.threshold_value
        )
        ...
        Log.objects.create(
                ...
                user=request.user,
                action=f'{consumable.name} details edited'
        )
        # Save the changes
        consumable.save()
        ...
        # Return a JSON response indicating success
        return JsonResponse({
            'message': f'Consumable with ID {id} updated successfully.'
        })

    # Return a JSON response indicating failure
    return JsonResponse(
            {'error': f'Error updating consumable with ID {id}.'},
            status=400
    )
...
def retrieve_consumable(request, id):
    ...
    if request.method == 'PUT':
        # Assuming the request body contains the updated consumable
        # data as JSON
        # Extract the data from the request body
        data = json.loads(request.body)
        ...
        if option == 'Pack size':
            ...
            if tmp == 0:
                ...
                Log.objects.create(
                        project=consumable.project,
                        user=request.user,
                        action=f'1 {consumable.name} taken from stock'
                )
            elif tmp > 0:
                ...
                Log.objects.create(
                        project=consumable.project,
                        user=request.user,
                        action=f'{amount} removed from {consumable.name} pack'
                )
            else:
                if abs(tmp) % consumable.pack_size == 0:
                    consumable.quantity = (
                            int(consumable.quantity) -
                            int(abs(tmp)/consumable.pack_size) - 1
                    )
                    consumable.pack_size_rem = consumable.pack_size
                    Log.objects.create(
                            project=consumable.project,
                            user=request.user,
                            action=(
                                f"{int(abs(tmp)/consumable.pack_size) - 1} "
                                f"{consumable.name} taken from stock")
                    )
                else:
                    consumable.quantity = (
                            int(consumable.quantity) -
                    ...
                    consumable.pack_size_rem = (
                            consumable.pack_size -
                            abs(tmp) % consumable.pack_size
                    )
                    Log.objects.create(
                            project=consumable.project,
                            user=request.user,
                            action=(f"{amount} removed from "
                                    f"{consumable.name} pack")
                    )
        else:
            ...
            Log.objects.create(
                    project=consumable.project,
                    user=request.user,
                    action=f'{amount} {consumable.name} taken from stock'
            )
        ...
        if consumable.quantity < consumable.threshold_value:
            email_thread = threading.Thread(
                    target=send_consumable_notification,
                    args=(consumable,)
            )
            email_thread.start()
        # Return a JSON response indicating success
...
def send_consumable_notification(consumable):
    ...
    recipient_list = [consumable.project.project_manager.email]
    ...
    email_images = ["Bell.png", "Logo_Orange.png", "Logo_White.png",
                    "facebook2x.png", "instagram2x.png", "linkedin2x.png",
                    "twitter2x.png"]
        ...
        if option == 'Pack size':
            ...
            if tmp <= consumable.pack_size:
                ...
                Log.objects.create(
                        project=consumable.project,
                        user=request.user,
                        action=f'{amount} added to {consumable.name} pack'
                )
            else:
                consumable.quantity = (int(consumable.quantity) +
                                       int(tmp//consumable.pack_size))
                ...
                Log.objects.create(
                        project=consumable.project,
                        user=request.user,
                        action=(f"{int(tmp/consumable.pack_size)} "
                                f"{consumable.name} added to  stock")
                )
        else:
            consumable.quantity = int(consumable.quantity) + int(amount)
            Log.objects.create(
                    project=consumable.project,
                    user=request.user,
                    action=f'{amount} {consumable.name} added to  stock'
            )
...
def deleteConsumable(request, project_name, pk):
    try:
        ...
        TrashConsumable.objects.create(
            project=project,
            name=consumable.name,
            product_code=consumable.product_code,
            pack_size=consumable.pack_size,
        ...
        Log.objects.create(
                project=consumable.project,
                user=request.user,
                action=f'{consumable.name} deleted'
        )
        return JsonResponse(
...
def deleteTrashConsumable(request, trash_consumable_id):
    ...
    Log.objects.create(
            project=trash_consumable.project,
            user=request.user,
            action=f'{trash_consumable.name} deleted from Trash'
    )
...
def delete_all_consumables_in_trash(request, project_name):
    ...
    Log.objects.create(
            project=project,
            user=request.user,
            action="Consumables trash emptied"
    )
    return JsonResponse({
        'message': 'All consumables in the trash have been deleted.'
    })
...
def restoreConsumable(request, project_name, trash_consumable_id):
    ...
    Log.objects.create(
            project=trash_consumable.project,
            user=request.user,
            action=f'{consumable.name} restored from Trash'
    )
    trash_consumable.delete()
...
def export_consumable_csv(request, project_name):
    ...
    writer.writerow(['Name', 'Product Code', 'Pack Size',
                     'Quantity', 'Expiry Date', 'Date Created',
                     'Storage Location', 'Threshold Value']
                    )

    user = request.user
    user_profile = UserProfile.objects.get(user=user)
    user_profile.managed_projects.all()
    project = user_profile.managed_projects.get(name=project_name)
    consumables = project.consumable_set.all()
...
def export_consumable_excel(request, project_name):
    ...
    headers = ['Name', 'Product Code', 'Pack Size', 'Quantity', 'Expiry Date',
               'Date Created', 'Storage Location', 'Threshold Value']
...
def export_consumable_txt(request, project_name):
    ...
    for consumable in consumables:
        ...
        content += (f"Pack Size: "
                    f"{consumable.pack_size_rem}/{consumable.pack_size}\n"
                    )
    ...
    try:
        if request.method == 'POST':
            ...
            Log.objects.create(
                    project=project,
                    user=request.user,
                    action=f'{name} added to Reagents'
            )
...
def dashboard_reagents(request, project_name):
    ...
    return render(
            request,
            'inventory/dashboard_reagents.html',
            {
                'plot_data': plot_data,
                'project': project,
                'projects': projects,
...
def edit_reagent(request, id):
    ...
    if request.method == 'PUT':
        ...
        reagent.expiry_date = data.get(
                'expiry_date ', reagent.expiry_date
        )
        reagent.storage_location = data.get(
                'storage_location', reagent.storage_location
        )
        reagent.threshold_value = data.get(
                'threshold_value', reagent.threshold_value
        )
        # Update other fields accordingly
        Log.objects.create(
                ...
                user=request.user,
                action=f'{reagent.name} details edited'
        )
        # Save the changes
        reagent.save()
        ...
        # Return a JSON response indicating success
        return JsonResponse({
            'message': f'Reagent with ID {id} updated successfully.'
        })

    # Return a JSON response indicating failure
    return JsonResponse(
            {'error': f'Error updating reagent with ID {id}.'},
            status=400
    )
...
def retrieve_reagent(request, id):
    ...
    if request.method == 'PUT':
        ...
        if option == 'Pack size':
            ...
            if tmp == 0:
                ...
                Log.objects.create(
                        project=reagent.project,
                        user=request.user,
                        action=f'1 {reagent.name} taken from stock'
                )
            elif tmp > 0:
                ...
                Log.objects.create(
                        project=reagent.project,
                        user=request.user,
                        action=f'{amount} removed from {reagent.name} pack'
                )
            else:
                if abs(tmp) % reagent.pack_size == 0:
                    reagent.quantity = (int(reagent.quantity) -
                                        int(abs(tmp)/reagent.pack_size) - 1)
                    reagent.pack_size_rem = reagent.pack_size
                    Log.objects.create(
                            project=reagent.project,
                            user=request.user,
                            action=(f"{int(abs(tmp)/reagent.pack_size) - 1} "
                                    f"{reagent.name} taken from stock")
                    )
                else:
                    reagent.quantity = (int(reagent.quantity) -
                                        int(abs(tmp)/reagent.pack_size) - 1
            ...
            Log.objects.create(
                    project=reagent.project,
                    user=request.user,
                    action=f'{amount} {reagent.name} taken from stock'
            )
        ...
        if reagent.quantity < reagent.threshold_value:
            email_thread = threading.Thread(
                    target=send_reagent_notification,
                    args=(reagent,)
            )
            email_thread.start()
        # Return a JSON response indicating success
...
def send_reagent_notification(reagent):
    ...
    # Add the email address to send the notification
    recipient_list = [reagent.project.project_manager.email]
        ...
        if option == 'Pack size':
            ...
            if tmp <= reagent.pack_size:
                ...
                Log.objects.create(
                        project=reagent.project,
                        user=request.user,
                        action=f'{amount} added to {reagent.name} pack'
                )
            else:
                reagent.quantity = (int(reagent.quantity) +
                                    int(tmp//reagent.pack_size))
                ...
                Log.objects.create(
                        project=reagent.project,
                        user=request.user,
                        action=(f"{int(tmp/reagent.pack_size)} "
                                f"{reagent.name} added to  stock")
                )
        else:
            reagent.quantity = int(reagent.quantity) + int(amount)
            Log.objects.create(
                    project=reagent.project,
                    user=request.user,
                    action=f'{amount} {reagent.name} added to  stock'
            )
...
def deleteTrashReagent(request, trash_reagent_id):
    ...
    Log.objects.create(
            project=trash_reagent.project,
            user=request.user,
            action=f'{trash_reagent.name} deleted from Trash'
    )
...
def delete_all_reagents_in_trash(request, project_name):
    ...
    Log.objects.create(
            project=project,
            user=request.user,
            action='Reagents trash emptied'
    )
    return JsonResponse({
        'message': 'All reagents in the trash have been deleted.'
    })
...
def restoreReagent(request, project_name, trash_reagent_id):
    ...
    Log.objects.create(
            project=trash_reagent.project,
            user=request.user,
            action=f'{reagent.name} restored from Trash'
    )
    trash_reagent.delete()
...
def addEquipment(request, project_name):
    try:
        if request.method == 'POST':
            ...
            # Create a new Equipment instance associated with the
            # active project
            Equipment.objects.create(
                    ...
                    quantity=quantity,
                    status=status,
                    service_contract_start=service_contract_start,
                    service_contract_end=service_contract_end,
            ...
            Log.objects.create(
                    project=project,
                    user=request.user,
                    action=f'{name} added to Equipment'
            )
...
def dashboard_equipment_(request, project_name):
    ...
    return render(
            request,
            'inventory/dashboard_equipment_.html',
            {
                'plot_data': plot_data,
                'project': project,
                'projects': projects,
...
def send_equipment_fault_notification(equipment):
    ...
    # Add the email address to send the notification
    project_manager_mail = [equipment.project.project_manager.email]
    ...
    # Render the HTML template as a string
    html_message = render_to_string(
            'inventory/non_functional_equipment_email_template.html',
            {'equipment': equipment}
    )
    msg = EmailMultiAlternatives(
            subject,
...
def edit_equipment(request, id):
    ...
    if request.method == 'PUT':
        ...
        equipment.service_contract_start = data.get(
                'service_contract_start', equipment.service_contract_start
        )
        equipment.service_contract_end = data.get(
                'service_contract_end', equipment.service_contract_end
        )
        equipment.donated_by = data.get('donated_by', equipment.donated_by)
        equipment.storage_location = data.get(
                'storage_location', equipment.storage_location
        )
        ...
        Log.objects.create(
                ...
                user=request.user,
                action=f'{equipment.name} details edited'
        )
        # Save the changes
        equipment.save()
        ...
        # Return a JSON response indicating success
        return JsonResponse({
            'message': f'Equipment with ID {id} updated successfully.'
        })

    # Return a JSON response indicating failure
    return JsonResponse(
            {'error': f'Error updating equipment with ID {id}.'},
            status=400
    )
...
def retrieve_equipment(request, id):
    ...
    if request.method == 'PUT':
        ...
        Log.objects.create(
                project=equipment.project,
                user=request.user,
                action=f'{quantity} {equipment.name} taken from stock'
        )
...
def return_equipment(request, id):
    ...
    if request.method == 'PUT':
        ...
        Log.objects.create(
                project=equipment.project,
                user=request.user,
                action=f'{quantity} {equipment.name} added to  stock'
        )
...
def deleteEquipment(request, project_name, pk):
    try:
        ...
        Log.objects.create(
                project=equipment.project,
                user=request.user,
                action=f'{equipment.name} deleted'
        )
        return JsonResponse(
...
def deleteTrashEquipment(request, trash_equipment_id):
    ...
    Log.objects.create(
            project=trash_equipment.project,
            user=request.user,
            action=f'{trash_equipment.name} deleted from Trash'
    )
...
def delete_all_equipment__in_trash(request, project_name):
    ...
    Log.objects.create(
            project=project,
            user=request.user,
            action='Equipment trash emptied'
    )
    return JsonResponse({
        'message': 'All equipment_ in the trash have been deleted.'
    })
...
def restoreEquipment(request, project_name, trash_equipment_id):
    ...
    Log.objects.create(
            project=trash_equipment.project,
            user=request.user,
            action=f'{equipment.name} restored from Trash'
    )
    trash_equipment.delete()
...
def export_equipment_csv(request, project_name):
    ...
    writer = csv.writer(response)
    writer.writerow(['Name', 'ID', 'Serial Number',
                     'Quantity', 'Status', 'Service Contract Start',
                     'Service Contract End', 'Donated', 'Storage Location'])
...
def export_equipment_excel(request, project_name):
    ...
    # Write headers to the sheet
    headers = ['Name', 'ID', 'Serial Number', 'Quantity',
               'Status', 'Service Contract Start',
               'Service Contract End', 'Donated', 'Storage Location']
    sheet.append(headers)
...
def export_equipment_txt(request, project_name):
    ...
    for equipment in equipment_:
        ...
        content += ("Service Contract Start: "
                    f"{equipment.service_contract_start}\n"
                    )
    ...
    try:
        if request.method == 'POST':
            ...
            Log.objects.create(
                    project=project,
                    user=request.user,
                    action=f'{sample_id} added to Samples'
            )
...
def dashboard_samples(request, project_name):
    ...
    return render(
            request,
            'inventory/dashboard_samples.html',
            {
                'plot_data': plot_data,
                'project': project,
                'projects': projects,
...
def edit_sample(request, id):
    ...
    if request.method == 'PUT':
        ...
        sample.storage_location = data.get(
                'storage_location', sample.storage_location
        )
        sample.threshold_value = data.get(
                'threshold_value', sample.threshold_value
        )
        # Update other fields accordingly
        Log.objects.create(
                ...
                user=request.user,
                action=f'{sample.sample_id} details edited'
        )
        # Save the changes
        sample.save()
        ...
        # Return a JSON response indicating success
        return JsonResponse({
            'message': f'Sample with ID {id} updated successfully.'
        })

    # Return a JSON response indicating failure
    return JsonResponse(
            {'error': f'Error updating sample with ID {id}.'},
            status=400
    )
...
def retrieve_sample(request, id):
    ...
    if request.method == 'PUT':
        ...
        Log.objects.create(
                project=sample.project,
                user=request.user,
                action=f'{amount} {sample.sample_id} taken from stock'
        )
        ...
        if sample.volume < sample.threshold_value:
            email_thread = threading.Thread(
                    target=send_sample_notification, args=(sample,)
            )
            email_thread.start()
        # Return a JSON response indicating success
        return JsonResponse({
...
def return_sample(request, id):
    ...
    if request.method == 'PUT':
        ...
        Log.objects.create(
                project=sample.project,
                user=request.user,
                action=f'{amount} {sample.sample_id} added to  stock'
        )
    ...
    # Return a JSON response indicating failure
    return JsonResponse(
            {'error': f'Error updating sample with ID {id}.'}, status=400
    )
...
def deleteSample(request, project_name, pk):
    try:
        ...
        Log.objects.create(
                project=sample.project,
                user=request.user,
                action=f'{sample.sample_id} deleted'
        )
        return JsonResponse(
                {'message': 'Sample deleted successfully'}, status=200
        )
...
def deleteTrashSample(request, trash_sample_id):
    ...
    Log.objects.create(
            project=trash_sample.project,
            user=request.user,
            action=f'{trash_sample.sample_id} deleted from Trash'
    )
...
def delete_all_samples_in_trash(request, project_name):
    ...
    Log.objects.create(
            project=project,
            user=request.user,
            action='Samples trash emptied'
    )
    return JsonResponse({
        'message': 'All samples in the trash have been deleted.'
    })
...
def restoreSample(request, project_name, trash_sample_id):
    ...
    Log.objects.create(
            project=trash_sample.project,
            user=request.user,
            action=f'{sample.sample_id} restored from Trash'
    )
    trash_sample.delete()
...
def export_sample_csv(request, project_name):
    ...
    writer = csv.writer(response)
    writer.writerow(['Sample ID', 'Type', 'Description', 'Country',
                     'Volume', 'Well ID', 'Date Recorded',
                     'Storage Location', 'Threshold Value'])
...
def export_sample_excel(request, project_name):
    ...
    # Write headers to the sheet
    headers = ['Sample ID', 'Type', 'Description', 'Country',
               'Volume', 'Well ID', 'Date Recorded',
               'Storage Location', 'Threshold Value']
    sheet.append(headers)
